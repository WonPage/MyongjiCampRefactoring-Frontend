/** 담당자 채윤 
 * 240618 - 이력서 추가 모달 UI 및 기능 구현 완료 */
import { useState } from "react";
import { Alert, Pressable, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
import { heightPercentageToDP as hp, widthPercentageToDP as wp } from "react-native-responsive-screen";
import useResume from "../hook/useResume";

export default function ResumeAddModal({navigation}){
    const [title, setTitle] = useState('');
    const [content, setContent] = useState('');
    const [url, setUrl] = useState('');
    const {postResume} = useResume();
    const handlePostResume = () => {
        ...
        postResume(resumeData).then(isFailed=>{
            ...
            <View style={{ width: wp('90%'), height: hp('72%'), backgroundColor: '#FFFEFA', 
                ...
                <View style={{flexDirection: 'row', justifyContent:'space-between', alignItems: 'center'}}>
                    <View style={{flexDirection:'row'}}>
                     <Text style={{fontSize:22, color:'#51493F', fontWeight:'500'}}>이력서 작성하기</Text>
                    </View>
                    <View style={{flexDirection:'row'}}>
                        <TouchableOpacity onPress={handlePostResume} style={{backgroundColor:'#88AB8E', width:wp('16%'), height:hp('5%'), justifyContent:'center', alignItems:'center', paddingBottom:hp('0.5%'), borderRadius:30}}>
